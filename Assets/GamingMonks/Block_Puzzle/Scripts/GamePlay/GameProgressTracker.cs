...
namespace GamingMonks
{
    ...
    public class GameProgressTracker : Singleton<GameProgressTracker>
    {
        ...
        private void OnEnable()
        {
            ...
            GamePlayUI.OnGameWinEvent += GamePlayUI_OnGameWinEvent;
        ...
        {
            if (GamePlayUI.Instance.currentGameMode != GameMode.Level && GamePlayUI.Instance.currentModeSettings.saveProgress) {
        ...
        /// <summary>
        /// Clears the progress of current gameplay on game over and unlocks next level
        /// </summary>
        private void GamePlayUI_OnGameWinEvent(GameMode currentMode)
        {
            ClearProgressData();
        }
        ...
        public void UpdateGameModePlayedCount(GameMode gameMode)
        {
            int count = GetGameModePlayedCount(gameMode);
            count++;
            PlayerPrefs.SetInt("gameModePlayedCount_" + gameMode, count);
        }

        public int GetGameModePlayedCount(GameMode gameMode)
        {
            return PlayerPrefs.GetInt("gameModePlayedCount_" + gameMode, 0);
        }

        public void UpdateFreqToWinLevel(int levelNumber)
        {
            int freq = GetFreqToWinLevel(levelNumber);
            freq++;
            PlayerPrefs.SetInt("freqToWinLevel" + levelNumber , freq);
        }

        public int GetFreqToWinLevel(int levelNumber)
        {
            return PlayerPrefs.GetInt("freqToWinLevel" + levelNumber, 0);
        }

        public void ResetFreqToWinLevel(int levelNumber)
        {
            PlayerPrefs.SetInt("freqToWinLevel" + levelNumber, 0);
        }
