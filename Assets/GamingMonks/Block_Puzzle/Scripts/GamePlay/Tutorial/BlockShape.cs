    ...
    public class BlockShape : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler
    {
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            if (eventData.pointerCurrentRaycast.gameObject != null)
            {
                ...
                if (clickedObject == thisTransform && canInteract)
                {
        ...
        bool TryPlacingShape()
        {
            ...
            foreach (Transform t in activeBlocks)
            {
                ...
                if (hittingBlocks.Count == activeBlocks.Count)
                {
                    ...
                    {
                        GamePlay.Instance.isChecked = false;
                        foreach(int rowId in completedRows)
                        {
                            List<Block> blocks = GamePlay.Instance.GetEntireRow(rowId);
                            foreach(Block block in blocks)
                            {
                                block.assignedSpriteTag = spriteTag;
                            }
                    ...
                    {
                        GamePlay.Instance.isChecked = false;
                        foreach(int columnId in completedColumns)
                        {
                            List<Block> blocks = GamePlay.Instance.GetEntirColumn(columnId);
                            foreach(Block block in blocks)
                            {
                                block.assignedSpriteTag = spriteTag;
                            }
                            GamePlay.Instance.ClearColumn(columnId);
                        }
